...
class RestorasControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create restora" do
    assert_difference('Restora.count') do
      post restoras_url, params: { restora: { location: @restora.location, name: @restora.name } }
    end
  ...
  test "should update restora" do
    patch restora_url(@restora), params: { restora: { location: @restora.location, name: @restora.name } }
